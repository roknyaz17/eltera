...
# 5. Библиотека тестов (обязательно) — 168 профилей: кандидаты, сотрудники, психометрика.
#    Идемпотентно: апсерт по ключу, повторный запуск и запуск на свежей БД не дублируют.
python -m app.seed_library

# 6. Демо-данные (необязательно)
python -m app.seed
